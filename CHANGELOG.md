...
### ✨ Features (新增特性)
- **整页工作区视觉改版**：
  - 统一顶部会话标签、左侧主机/文件导航、终端环境栏、AI 助手面板与底部状态栏的视觉层级。
  - 窄窗口下 AI 助手改为右侧抽屉，避免压缩终端宽度；移除重复的助手开关。
- **Agent 审批与失败恢复优化**：
  - 可一次批准当前计划中同工具、同风险规则的明确步骤；审批卡片列出适用步骤，执行前仍重新校验。
  - 计划失败后可从失败步骤继续，保留已完成步骤和输出。
- **助手交互改进**：只读查询自动执行；修复计划失败状态残留、流式输出滚动跟随和终端动作造成的外层滚动问题。

### 🧪 Tests & Docs (测试与文档)
- 提供中英文 README，并在两份文档间加入语言切换链接。
- 移除 `docs/CODE-QUALITY-REVIEW.md`。
- 类型检查通过。
